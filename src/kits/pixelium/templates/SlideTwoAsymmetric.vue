...
<template>
  <div class="px-two-asym flex items-center justify-center w-full h-full" style="background: var(--color-background)">
    <div class="relative z-10 w-full max-w-5xl px-8">
      <h3 class="px-asym-title text-center">{{ slide.title || 'Two Column' }}</h3>
      ...
      <div v-if="parsed.subtitle" class="px-asym-sub mb-6 text-center" v-html="parsed.subtitle" />

      <div class="flex gap-8" :style="{ gridTemplateColumns: undefined }" :class="`px-asym-grid-r${ratio.l}-${ratio.r}`">
        <div v-for="(col, i) in parsed.columns" :key="i" style="background: var(--color-card); border: 2px solid var(--color-border); box-shadow: 4px 4px 0 #0006; padding: 1.5rem 2rem; position: relative;" :style="{ flex: i === 0 ? ratio.l : ratio.r }">
          <!-- 顶部色条 -->
          <div style="position: absolute; top: 0; left: 0; right: 0; height: 4px; background: var(--color-accent);" />
          <h4 class="px-asym-col-title">{{ col.title }}</h4>
          <div v-html="col.html" class="px-asym-col-body" />
        </div>
      </div>
    </div>
  </div>
...
.px-asym-col-body :deep(ul),
.px-asym-col-body :deep(ol) {
  padding-left: 1.25rem;
  margin-bottom: 0.5rem;
}

.px-asym-col-body :deep(li) {
...
.px-asym-col-body :deep(li::marker) {
