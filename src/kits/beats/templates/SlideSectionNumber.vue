...
.wrap { max-width:min(calc(var(--content-max-narrow) * var(--content-scale)),100%); min-width:min(calc(44rem * var(--content-scale)),100%); margin:0 auto; }
.card {
  padding:3rem 4rem; text-align:center; border-radius:1.5rem; min-width:min(calc(44rem * var(--content-scale)),100%);
...
.num { display:block; font-size:calc(7rem * var(--content-scale)); font-weight:900; line-height:1; background:linear-gradient(135deg,var(--color-accent),var(--color-h1-to,#647EFF)); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; opacity:0.2; }
